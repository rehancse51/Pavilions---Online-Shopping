...
class Super_admin {
    ...
    public function save_admin_info($data){
       ...
       $sql = "INSERT INTO tbl_admin(admin_name,email_address,password) VALUES('$data[admin_name]','$data[email_address]','$password')";
        if (mysqli_query($this->db_connect, $sql)) {
            $message = "Admin registation successfully";
            return $message;
        } else {
    ...
    public function select_all_product_info(){
        $sql = "SELECT p.product_id,p.product_name, p.category_id,p.manufacturer_id,p.product_price,p.stock_quantity,p.publication_status,c.category_name,m.manufacturer_name FROM tbl_product as p, tbl_category as c,tbl_manufacturer as m WHERE p.category_id=c.category_id AND p.manufacturer_id=m.manufacturer_id ORDER BY product_id DESC";
        if (mysqli_query($this->db_connect, $sql)) {
            $query_result = mysqli_query($this->db_connect, $sql);
            return $query_result;
        } else {
    ...
    public function select_all_order_info(){
         $sql = "SELECT o.* , c.first_name , c.last_name FROM tbl_order as o , tbl_customer as c , tbl_payment as p WHERE o.customer_id = c.customer_id AND o.order_id=p.order_id ORDER BY order_id  DESC ";
        if (mysqli_query($this->db_connect, $sql)) {
            $query_result = mysqli_query($this->db_connect, $sql);
            return $query_result;
        } else {
...
//    }
    ...
    public function delete_order_info($order_id){
        ...
        if (mysqli_query($this->db_connect , $sql)) {
            $message = "order info successfully deleted";
            return $message;
        } else {
            die("Query Problem" . mysqli_error($this->db_connect));
        }
